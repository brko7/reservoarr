...
| `RESV_REPLAY_SKIP` | `0` | `1` drops what an edge resends after a plain reconnect: the fetcher remembers the last PCR taken before the seam and discards packets of the new connection until the PCR passes it, so the same seconds are not played twice and the replay is not banked as new cushion. Passes everything through, as before, when the replay starts further back than `RESV_REPLAY_MAX_S`, carries a discontinuity flag, stops advancing, or shows no PCR within 1 MB; never after a flushing reconnect. |
