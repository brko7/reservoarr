...
- **New `RESV_TS_CHANNEL={channelId}` tunable that keeps a channel's output timeline continuous across processes.** ffmpeg rebases its output to start near PTS 0, so every process Dispatcharr spawns for a channel — a failover to the next stream, the fallback slate, the switch back from it — starts the timeline over. Downstream the switch arrives inside one client connection as a PTS jump back by however long the previous process ran. Jellyfin's live transcoder runs with `-copyts` and drops every frame that is older than what it has already written, so it stops producing HLS segments until the new timestamps catch up. Measured on Dispatcharr 0.31.0 with Jellyfin 12.0.0, a return from a 2-minute fallback slate: slate at PTS 120.1s, next stream from 0.12s, the transcoder logged `Non-monotonic DTS; previous: 11481600, current: 960`, its frame counter stayed at 632, and all 30 test viewers stalled for about as long as the slate had played. With the tunable set, each process reads `{RESV_LOG_DIR}/pts-<id>.json`, projects the previous process's last output PTS to now, and starts one second past it with `-output_ts_offset`; the first process of a channel starts on the wall clock. The last output PTS is read off ffmpeg's own output, which now goes through a relay thread to stdout byte for byte, and is written back every 2s and on exit, under an `flock` on `pts-<id>.json.lock`: when two processes overlap during a switch, the later timeline is kept. The relay is drained before the last write. An output PTS more than 30s ahead of where the wall clock puts it is ignored, the way the PCR clock already rejects garbage samples, so one corrupt PES header cannot push the timeline hours ahead for every later process; a state file with a non-finite or absurd end falls back to the wall clock. Three processes in a row on `cdn_sim`:
...
- Not `-copyts` (invariant #5): ffmpeg still rebases the input, and the offset moves every output stream by the same amount. The tunable has to be set per channel, in the profile: Dispatcharr substitutes `{channelId}` anywhere in the parameters, e.g. command `/usr/bin/env` and parameters `RESV_TS_CHANNEL={channelId} /data/reservoarr/reservoarr.py {streamUrl} {userAgent}`. The profile the plugin generates does not set it; `docs/TUNABLES.md` and the README say how to edit it.

- `docs/TUNABLES.md`, `docs/TELEMETRY.md` (the two `ts timeline:` lines), the README and `docs/INVARIANTS.md` (#2, #5) updated. New tests: 21 in `tests/unit/test_ts_timeline.py` (opt-in, command, channel id sanitising, projection, stale, corrupt and non-finite state, PES PTS parsing, 33-bit wrap, output behind the wall clock, the 30s plausibility gate, the later writer winning, the lock, the relay), and 2 e2e in `tests/e2e/test_ts_timeline.py` that run two processes back to back with and without the tunable.
...
- `docs/TELEMETRY.md` documents the two `replay after reconnect: …` lines; `docs/TUNABLES.md` the two tunables. New tests in `tests/unit/test_replay_skip.py`: 21 for the gate, 1 for the chunked ingest and 3 for the arrival counter and the watchdog; and an e2e on the forced-EOF path with the gate on (`run_pipeline(replay_skip=True)`).
