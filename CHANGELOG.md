...
- `docs/TUNABLES.md`, `docs/TELEMETRY.md` (the `ts timeline:` line) and `docs/INVARIANTS.md` (#2, #5) updated. New tests: 17 in `tests/unit/test_ts_timeline.py` (opt-in, command, channel id sanitising, projection, stale and corrupt state, PES PTS parsing, 33-bit wrap, the later writer winning, the lock, the relay), and 2 e2e in `tests/e2e/test_ts_timeline.py` that run two processes back to back with and without the tunable.
